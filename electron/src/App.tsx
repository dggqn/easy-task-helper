...
const projects: Project[] = [
  ...
  { id: 'merchant-portal', name: '商户服务门户', path: 'D:\\workspace\\merchant-portal', framework: 'Vue + Vite', status: 'review' },
...
function App() {
  ...
  async function sendMessage() {
    ...
    if (!content) return
    const id = Date.now()
    setMessages((current) => [...current, { id: `user-${id}`, role: 'user', content }, { id: `assistant-${id}`, role: 'assistant', content: '已记录任务，正在生成可审查的执行计划。' }])
    setDraft('')
    try {
      const result = await window.ethHarness?.runTask({ project_name: selectedProject?.name ?? '', project_path: selectedProject?.path ?? '', instruction: content })
      if (!result) return
      setMessages((current) => current.map((message) => message.id === `assistant-${id}` ? { ...message, content: `${result.summary}\n${result.plan.map((step, index) => `${index + 1}. ${step}`).join('\n')}` } : message))
    } catch (error) {
      const detail = error instanceof Error ? error.message : '未知错误'
      setMessages((current) => current.map((message) => message.id === `assistant-${id}` ? { ...message, content: `Harness 执行失败：${detail}` } : message))
    }
  }
  ...
  if (!selectedProject) return <main className="app-shell project-home">
    <header className="topbar"><div className="brand" aria-label="Easy Task Helper"><span className="brand-mark">ETH</span><span>Easy Task Helper</span></div><span className="phase-badge">PHASE 3 / 本地工作台</span></header>
  ...
  return <main className="app-shell task-home">
    <div className="action-toolbar"><button type="button" onClick={() => runAction({ action: 'inspect' })}>检查项目</button><button type="button" onClick={() => runAction({ action: 'run-command', name: 'python-tests' })}>运行测试</button></div>
    {isActionResultOpen && actionResult && <section className="action-result-panel" aria-label="执行结果"><div className="action-result-header"><strong>执行结果</strong><button type="button" aria-label="关闭执行结果" onClick={() => setIsActionResultOpen(false)}>关闭</button></div><pre>{actionResult}</pre></section>}
